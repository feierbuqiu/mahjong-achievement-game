...
def main (args : List String) : IO UInt32 := do
  ...
  for raw in data.splitOn "\n" do
    ...
    let ok := FlatRules.safeB tables packed
    let successors := if mode == "successors" then (allTiles.filter (fun u =>
      PackedRules.available tables packed u && FlatRules.safeB tables (PackedRules.add tables packed u))).length else 0

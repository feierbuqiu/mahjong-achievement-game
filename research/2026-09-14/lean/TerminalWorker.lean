...
def main (args : List String) : IO UInt32 := do
  ...
  if !TerminalBucket.span tables d bits start count then
    for i in [start:start+count] do
      if !TerminalBucket.row tables d bits i then
  ...
  let result := "{\"schema\":\"LEAN_TERMINAL_BUCKET_CHUNK_V1\",\"status\":\"PASS\",\"start\":" ++
  ...
  IO.println s!"PASS_TERMINAL_BUCKET start={start} count={count} nanoseconds={elapsed}"
  return 0
